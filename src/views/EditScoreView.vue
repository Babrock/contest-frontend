...
export default {
  ...
  beforeMount() {
    this.axios.get(`http://localhost:8080/users/details`).then((response) => {
      ...
      if(data.region == null){
        alert("Użytkownik nie ma przypisanego regionu.")
      } else{
        this.user.region.id = data.region.id
        this.axios.get(`http://localhost:8080/form/region-id/${this.user.region.id}`).then((response) => {
  ...
  methods: {
    async fetchData() {
      await this.axios.get(`http://localhost:8080/tasks/sum-score`).then((response) => {
        this.taskswithsumscore = responseSumScore.data
      })
      await this.axios.get(`http://localhost:8080/tasks`).then((response) => {
        this.tasks = response.data
      })
      // przez zamianę wyskakuje błąd
    ...
    async saveScore() {
      ...
      try {
        ...
        this.close()
        await this.fetchData()     //JAKIŚ PROBLEM NAPRAWIĆ!!!
      } catch (error) {
        if (error.response && error.response.status === 500) {
          alert("Zadanie o takiej nazwie już istnieje.");
        } else {
